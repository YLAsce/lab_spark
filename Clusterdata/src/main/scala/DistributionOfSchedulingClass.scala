...
object DistributionOfSchedulingClass {
   def main(args: Array[String]) = {
      ...
      val files_job_events = Files.walk(Paths.get("./src/main/data/job_events"), FileVisitOption.FOLLOW_LINKS).toArray.map(x => x.toString())

      val files_task_events = Files.walk(Paths.get("./src/main/data/task_events"), FileVisitOption.FOLLOW_LINKS).toArray.map(x => x.toString())
